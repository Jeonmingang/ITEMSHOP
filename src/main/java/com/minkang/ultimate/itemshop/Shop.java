...
public class Shop {
    ...
    private final Map<Integer, ShopItem> items = new HashMap<Integer, ShopItem>();

    public Shop(String name, int size) { this.name = name; this.size = size; }
    ...
    public Inventory createInventory() {
        ...
        for (Map.Entry<Integer, ShopItem> e : items.entrySet()) {
            int slot = e.getKey();
            if (slot >= 0 && slot < size) {
                inv.setItem(slot, Util.withPriceLore(e.getValue().getItem().clone(), e.getValue().getPrice(), currency));
            }
        }
        return inv;
    }
