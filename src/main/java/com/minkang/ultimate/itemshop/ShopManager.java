...
public class ShopManager {
    ...
    // npc 이름 -> 상점 이름
    private final Map<String, String> npcLinks = new HashMap<>();
    ...
    public void save() {
        ...
        for (Shop shop : shops.values()) {
            ...
            for (Map.Entry<Integer, ShopItem> e : shop.getItems().entrySet()) {
        ...
        for (Map.Entry<String, String> e : npcLinks.entrySet()) {
            data.set("npc_links." + e.getKey(), e.getValue());
        }
        try {
            data.save(file);
        } catch (IOException ex) {
            plugin.getLogger().warning("shops.yml 저장 실패: " + ex.getMessage());
        }
    }

    public boolean exists(String name) {
        return shops.containsKey(name.toLowerCase(Locale.ROOT));
    }
    ...
    public Shop create(String name) {
        Shop shop = new Shop(name, 54);
        shops.put(name.toLowerCase(Locale.ROOT), shop);
        return shop;
    }

    public boolean delete(String name) {
        return shops.remove(name.toLowerCase(Locale.ROOT)) != null;
    }
    ...
    public void linkNpc(String npcName, String shopName) {
        npcLinks.put(npcName, shopName);
    }

    public void unlinkNpc(String npcName) {
        npcLinks.remove(npcName);
    }

    public String getLinkedShopName(String npcName) {
        return npcLinks.get(npcName);
    }

    public Map<String, String> getNpcLinks() {
        return npcLinks;
    }
}
