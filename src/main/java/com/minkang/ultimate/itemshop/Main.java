...
public class Main extends JavaPlugin {
    ...
    public static String TITLE_PREFIX = "§a아이템상점: ";
...
Bukkit.getPluginManager().registerEvents(new GuiListener(this), this);
    ...
    public void reloadFromDisk() {
        ...
        if (shopManager == null) shopManager = new ShopManager(this);
        shopManager.load();
    }
    ...
    public void playBuyEffect(org.bukkit.entity.Player p) {
        if (getConfig().getBoolean("effects.play_sound", true)) {
            ...
            try { p.playSound(p.getLocation(), Sound.valueOf(s), 1.0f, 1.0f); } catch (IllegalArgumentException ignored) {}
        }
    }
}
